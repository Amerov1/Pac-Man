...
public class Cheese extends Entity {
    ...
    public void CreateCheese() {
        ...
        for (int row = 0; row < _map.length; row++) {
            for (int col = 0; col < _map[0].length; col++) {
                if (_map[row][col] == 3) { // Nur Zellen mit "0" sind f�r K�se
                    int x = col * width;  // Spalte -> Pixel-X
                    int y = row * height; // Zeile -> Pixel-Y
                    shapes.add(new Point(x, y));        // K�seposition in Pixeln speichern
                }
            }
        }
    }
    ...
    public void check() {
        shapes = shapes.stream().filter(i -> {
            // �berpr�fe, ob `x` mit dem Spieler �berlappt
            ...
            if (isOverlapping) {
                player.increaseScore(); // Punkte erh�hen, wenn �berlappung erkannt wird
